...
class _QrScannerScreenState extends State<QrScannerScreen> with SingleTickerProviderStateMixin, WidgetsBindingObserver {
  ...
  void _onDetect(BarcodeCapture capture) {
    ...
    if (barcodes.isNotEmpty && barcodes.first.rawValue != null) {
      ...
      String targetAccount = result;
      double? targetAmount;
      String? targetDescription;

      if (result.startsWith('banke://payment')) {
        final uri = Uri.tryParse(result);
        if (uri != null) {
          targetAccount = uri.queryParameters['account'] ?? result;
          final amtStr = uri.queryParameters['amount'];
          if (amtStr != null) {
            targetAmount = double.tryParse(amtStr);
          }
          targetDescription = uri.queryParameters['description'];
        }
      } else if (result.startsWith('EG90002000000000')) {
        targetAccount = result.replaceAll('EG90002000000000', '').replaceFirst(RegExp(r'^0+'), '');
      } else if (result.startsWith('wallet_')) {
        targetAccount = result.replaceAll('wallet_', '');
      }

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(AppLocalizations.of(context)!.qrDetected),
          ...
          Navigator.pushReplacement(
            ...
            MaterialPageRoute(
              builder: (context) => TransferScreen(
                ...
                prefilledAmount: targetAmount,
                prefilledDescription: targetDescription,
              ),
            ),
          );
        }
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: isActive ? Colors.transparent : Colors.white.withValues(alpha: 0.1)),
          ),
