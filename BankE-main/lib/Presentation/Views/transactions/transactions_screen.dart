...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  void _downloadStatement() {
    if (_selectedDateRange == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select a date range first using the calendar icon')),
      );
    ...
    return Scaffold(
      ...
      body: BlocListener<TransactionBloc, TransactionState>(
        listener: (context, state) {
          if (state is TransactionStatementDownloaded) {
            Navigator.push(
              context,
              MaterialPageRoute(
  ...
  Widget _filterChip(String label) {
    ...
    return ChoiceChip(
      ...
      side: BorderSide(color: isSelected ? theme.primaryColor : theme.dividerColor.withValues(alpha: 0.1)),
      showCheckmark: false,
    );
  }
    ...
    return BlocBuilder<TransactionBloc, TransactionState>(
      builder: (context, state) {
        if (state is TransactionLoading) {
          return const ShimmerLoadingWidget(type: 'list');
          ...
          if (filteredTransactions.isEmpty) {
            ...
            return Center(
              child: Column(
                ...
                children: [
                  Icon(Icons.history_rounded, size: 64, color: theme.dividerColor.withValues(alpha: 0.2)),
