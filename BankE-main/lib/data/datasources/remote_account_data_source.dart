...
class RemoteAccountDataSourceImpl implements AccountDataSource {
  ...
  @override
  Future<void> updateCardControls(String cardId, bool online, bool atm, bool international) async {
    final response = await cardsService.updateControls(int.parse(cardId), online, atm, international);
    apiClient.ensureSuccess(response);
  }

  @override
  Future<void> changeCardPin(String cardId, String pin) async {
    final response = await cardsService.changePin(int.parse(cardId), pin);
    apiClient.ensureSuccess(response);
  }
  ...
  @override
  Future<Uint8List> downloadStatement(DateTime startDate, DateTime endDate) async {
    final response = await accountService.getStatement(startDate, endDate);
    apiClient.ensureSuccess(response);
    return response.data as Uint8List;
  }

  @override
  Future<Map<String, dynamic>> fetchHomeDashboardStats() async {
    final response = await accountService.getDashboardStats();
    apiClient.ensureSuccess(response);
