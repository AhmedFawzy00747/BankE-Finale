...
namespace BankE.API.Controllers
{
    ...
    public class CardsController : ControllerBase
    {
        ...
        [HttpPut("{id}/controls")]
        public async Task<IActionResult> UpdateControls(int id, UpdateCardControlsRequest request)
        {
            var result = await _cardsService.UpdateControlsAsync(CurrentUserId, id, request);
            if (!result.Success) return BadRequest(result);
            return Ok(result);
        }

        [HttpPut("{id}/pin")]
        public async Task<IActionResult> ChangePin(int id, ChangeCardPinRequest request)
        {
            var result = await _cardsService.ChangePinAsync(CurrentUserId, id, request);
            if (!result.Success) return BadRequest(result);
            return Ok(result);
        }
    }
